...
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<< DRONE CONNECTIONS AND RELATED CONTROL FUNTION >>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
# DroneKit movement commands
def send_ned_velocity(vehicle, north, east, down, duration):
    """
    Send velocity commands to the drone only if it is airborne.
    """
    if not is_drone_airborne(vehicle):
        print("Drone is not airborne. Velocity commands will not be sent.")
        return
    ...
    try:
        msg = vehicle.message_factory.set_position_target_local_ned_encode(
            ...
            north, east, down,  # vx, vy, vz in m/s
            0, 0, 0,  # ax, ay, az (not supported yet, ignored in GCS_Mavlink)
...
def is_drone_airborne(vehicle):
    """
    Checks if the drone is airborne based on its current altitude.
    """
    if vehicle.location.global_relative_frame.alt > 0.5:  # Adjust threshold as needed
        return True
    return False


# <<<<<<<<<<<<<<<<<<<<<<<<<<<<< OBJECT TRACKING AND RE-IDENTIFICATION RELATED FUNTIONS >>>>>>>>>>>>>>>>>>>>>>>>>>>>> #
...
# <<<<<<<<<<<<<<<<<<<<<<<< MOUSE CLICK EVENTS WHEN WORKING WITH MY QGROUNDCONTROL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>#
...
def video_feed_thread():
    ...
    prev_time = time.time()
    fps = 0
    scale_factor = 0.01  # Adjust for movement sensitivity
            ...
            with frame_lock:
                if rectangle is not None:
                    ...
                    if confidence > 5:
                        ...
                        # Calculate offsets and send commands to drone
                        if is_drone_airborne(vehicle):
                            print("Drone is airborne, calculating movement...")
                            offset_x = center_x - DESIRED_WIDTH // 2
                            offset_y = center_y - DESIRED_HEIGHT // 2
                            north_velocity = -offset_y * scale_factor
                            east_velocity = offset_x * scale_factor
                            print(f"Drone movement: North_Velocity: {north_velocity}, East_Velocity: {east_velocity}..")
                            send_ned_velocity(vehicle, north_velocity, east_velocity, 0, 1)
                        else:
                            print("Drone is not airborne. Skipping velocity commands.")

                    # If confidence is low (tracking failed)
                    else:
                        print("Tracking failed.")

                # Start re-identification process if not already running
                if not tracking_initialized and not reidentifying:
                    print("Starting re-identification process.")
                    with lock:
                        reidentifying = True
                    reid_thread = threading.Thread(target=re_identification_thread, daemon=True)
                    reid_thread.start()

            # Display FPS and tracker status
