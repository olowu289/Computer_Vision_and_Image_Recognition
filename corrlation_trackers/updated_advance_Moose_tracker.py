...
# Function to compute color histogram in high pixel intensity regions
...
# Function to compute ORB features within the bounding box
...
# Function to parallelize the search across the frame using sliding windows
...
# Function to generate sliding window bounding boxes
...
# Re-identification thread that searches across the entire frame
    ...
    while True:
        ...
        if orb_active and initial_descriptors is not None:
            ...
            if found_bbox:
                ...
                tracker = cv2.TrackerBoosting_create()  # Changed to Boosting tracker
            ...
            if command.startswith('p'):
                ...
                with lock:
                    ...
                    if tracking_initialized and is_point_inside_rectangle(center_x, center_y, rectangle):
                        ...
                        tracker = cv2.TrackerBoosting_create()  # Ensure the tracker is initialized here
                        if tracker is not None:  # Check if the tracker is successfully created
                            tracking_initialized = False
                            color = (255, 0, 0)
            ...
            with frame_lock:
                if rectangle is not None:
                    if not tracking_initialized:
                        # Initialize the tracker with the rectangle only if the tracker exists
                        if tracker is not None:
                            tracker.init(frame, tuple(rectangle))
                            tracking_initialized = True
                        else:
                            print("Tracker is not initialized. Skipping tracking.")
