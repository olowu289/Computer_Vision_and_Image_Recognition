...
# Function to compute ORB features
...
# Calculate confidence score based on similarity and matches
def calculate_confidence(similarity, matches, max_matches):
    match_confidence = min(len(matches) / max_matches, 1.0)
    confidence = (similarity + match_confidence) / 2.0
    return confidence
...
def search_entire_frame(frame, initial_hist, initial_keypoints, initial_descriptors, bf):
    ...
    # Reduced similarity threshold and match count for the entire frame
...
def parallel_search(frame, bboxes, initial_hist, initial_keypoints, initial_descriptors, bf):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(search_entire_frame, frame, bbox, initial_hist, initial_keypoints, initial_descriptors, bf) for bbox in bboxes]
...
def generate_sliding_windows(frame_width, frame_height, window_size=100, step_size=15):
    ...
    while True:
        ...
        if orb_active and initial_descriptors is not None:
            ...
            if found_bbox:
                ...
                tracker.start_track(current_frame, tuple(found_bbox))  # Reinitialize tracker with new bounding box
...
# Updated tracking code in the video_feed_thread function
        ...
        if ret:
            with frame_lock:
                if rectangle is not None:
                    ...
                    if confidence > 7:  # Threshold for successful tracking (adjust as needed)
                        ...
                        if high_pixel_mask is not None and roi is not None:
                            # Show the high pixel intensity mask in one window
                            cv2.imshow('High Pixel Intensity Mask', high_pixel_mask)

                            # Show the corresponding RGB image in another window
                            cv2.imshow('RGB Region', roi)

                            # Display the pixel values (optional, for debugging purposes)
                            print(f"High pixel intensity values: {high_pixel_values}")
